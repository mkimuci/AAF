function UCIaudapterDemo_online(mode, myplot, varargin)
audioInterfaceName = 'Focusrite USB';
global F1F2angle F1F2amplitude ToCalibrate key recordDuration gender;
...
%Audapter('setParam', 'frameLen', 64, 0);  %%%%%%%% KS set to 64 to match F0 shift   %%%  KS
...
if isequal(mode, 'persistentFormantShift')
    ...
    signphi = F1F2angle; 
    %signphi=0; 
    %if UCIF1<0,signphi=2*pi;end  %shift F1 down

    params.f1Min = 0;
    params.f1Max = 5000;  
    params.f2Min = 0;
    params.f2Max = 5000;  
    params.pertF2 = linspace(0, 5000, 257);   %INDEPENDENT VARIABLE OF THE PERTURBATION FIELD (0 to 5000 HZ)
    params.pertAmp = F1F2amplitude * ones(1, 257);
    %params.pertPhi = signphi + zeros(1, 257);
    params.pertPhi = signphi * ones(1, 257);
    params.bTrack = 1;
    params.bShift = 1;
...
%     params.rampLen = 0.25;
   ...
   if ToCalibrate
        tic;
        timer = 0;
        while ~key && timer<recordDuration
            timer = toc;
            pause(0.05);
        end
    else
   ...
   fprintf(1, '\n');
    ...
    bVis=myplot; % added by KS (if check box in AAFdemo is on, then myplot=1, and bVis will plot  ADDED BY KS
    bVisFmts = 1;
    visName = 'Persistent formant shift';
    
elseif isequal(mode, 'focalFormantShift')    %FOCAL FORMANT SHIFT (KS)
    gender = varargin{1};
...
elseif isequal(mode, 'persistentPitchShift')         %PERSISTENT PITCH SHIFT (KS) Whole spectrum shift: p. 10
...
if bVis
    ...
    %-----------------------%
    %figure;
    figure('units','normalized','pos',[0.005    0.25    0.25    0.35]);  %%%%%%%%%%%KS
    ...
    xs = get(gca, 'XLim');
    ys = get(gca, 'YLim');
    text(xs(1) + 0.025 * (max(xs)-min(xs)), ys(2) - 0.075 * (max(xs)-min(xs)), ...
